...
Pipeline: `scanned page -> character detection -> character classification -> per-character stylization (alpha) -> page recomposition -> web app`
...
| Web app (React + TS frontend, FastAPI sessions API) | ✅ | `ft/web-ui` |
...
Web app: start the API (it serves the built frontend when present) and, for development, the Vite dev server with hot reload:

```
uv run api                 # FastAPI + pipeline sessions; override with HOST/PORT env vars
```

```
cd web && npm install && npm run dev   # UI at http://127.0.0.1:5173 (proxies /api to :8000)
```

To serve the production build from the API alone:

```
cd web && npm run build    # outputs web/dist, which `uv run api` serves automatically
```
...
The web app mirrors all of this: its "Quills & inks" panel exposes the same
options as the CLI flags (detector `mser`/`yolo` with weights path and confidence,
optional classifier checkpoint, and stylizer `baseline`/`neural`/`latent` with
its model paths and the alphabet dataset). Model paths are resolved on the
machine running the API.

# Project structure

The repository is a [uv workspace](https://docs.astral.sh/uv/concepts/workspaces/) split into a Python backend and a TypeScript web frontend:
...
├── backend/
│   ├── pyproject.toml        # call-ai-grapher package + CLI/API entry points
│   ├── experiments/          # early GAN/denoising experiment scripts
│   └── src/call_ai_grapher/  # pipeline, models, dataset, api, object detection...
├── web/
